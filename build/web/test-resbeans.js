...
function TestSupport() {
    this.wadlDoc = null;
    this.wadlURL = '';
    this.wadlErr = 'Não é possível acessar WADL: reinicie seu aplicativo RESTful e atualize esta página.';
    ...
    this.viewIds = [
        { "id" : "table" , "name":"Exibição tabular", "type":"tableContent"}, 
        { "id" : "raw" , "name":"Exibição bruta", "type":"rawContent"}, 
...
TestSupport.prototype = {    
    ...
    initFromWadl : function () {
        ...
        if(wDoc != null) {
            ...
            this.updatepage('content', '<span class=bld>Página da ajuda</span><br/><br/><p>Não é possível acessar WADL: reinicie seu aplicativo REST e atualize esta página.</p><p>Se você ainda estiver vendo este erro e se estiver acessando esta página através do plug-in Firefox com Firebug, então<br/>você precisa desativar o firebug para arquivos locais. Esta é a barra de menus do Firefox, selecione <br/>Ferramentas > Firebug > Desativar firebug para arquivos locais</p>');
        }            
    },
    ...
    clearAll : function() {
        this.clearOutput();
        this.updatepage('request', 'Selecione um nó na barra de navegação (no lado direito desta página) para testar.');
    ...
    monitor : function (method, xmlHttpReq, param) {
        ...
        var s = 'Solicitação: ' + this.currentMethod + ' ' + cURL + '\n<br/>' + params +
                    ...
                    '\n\n<br/><br/>Carimbo de data/hora: ' + ' ' + xmlHttpReq.getResponseHeader('Date') + '';
        var prevs = '';
    ...
    updateContent : function (content) {
        var showRaw = true;
        var cErr = 'O conteúdo talvez não tenha a relação conteúdo-continente. Veja a exibição bruta do conteúdo.';
    ...
    debug : function(message) {
        ...
        if(dbgComp == null) {
            ...
            dbgComp.innerHTML = '<table><tr><td><span style="color: blue">Janela Depuração Rest</span></td><td>'+addActionStr + '</td></tr></table><br/>';
        }
    ...
    getHttpRequest : function() {
        ...
        {    // Internet Explorer 6.0+, 5.0+
            ...
            catch (e)
            {
                ...
                {
                    ts.debug("Seu navegador não suporta AJAX!");
                }
            }
        }
        return xmlHttpReq;
        ...
        try {
            ...
            if (this.isResponseReady(method, xmlHttpReq, content, monitor)) {
              ...
              if(rtext == undefined || rtext == '' || rtext.indexOf('HTTP Status') != -1) {
                  var err = method+' SolicitaçãoFailed RequestFailed --> Status: (' + status+')\n<br/>'+
